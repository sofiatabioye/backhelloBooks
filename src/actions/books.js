...
export function borrowBook(userId, bookId) { // eslint-disable-line require-jsdoc
    return (dispatch) => {
        dispatch({ type: 'BORROW_BOOK_BEGINS' });
        return axios.post(`/api/v1/users/${userId}/books/${bookId}/borrow`)
            .then((response) => {
                dispatch({ type: 'BORROW_BOOK_SUCCESS', books: response.data });
...
export function returnBook(userId, bookId) { // eslint-disable-line require-jsdoc
    return dispatch => axios.put(`/api/v1/users/${userId}/books/${bookId}/return`);
}
